...
const CylinderBuyerManagement = () => {
  const { buyers, addBuyer, updateBuyer, deleteBuyer } = useCylinderBuyers();
  ...
  const handleSave = (buyer: Buyer) => {
    ...
    setEditingId(null);
    setEditForm({});
    toast.success('Cylinder buyer updated successfully');
  };
  ...
  const handleDelete = (gstin: string) => {
    if (window.confirm('Are you sure you want to delete this cylinder buyer? This action cannot be undone.')) {
      deleteBuyer(gstin);
      toast.success('Cylinder buyer deleted successfully');
    }
  };
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">Cylinder Buyer Management</h2>
          <Button onClick={handleAdd}>
            <Plus className="h-4 w-4 mr-2" />
            Add New Cylinder Buyer
          </Button>
        </div>
...
export default CylinderBuyerManagement;
